...
if fs.exists(file) ~= true then
  ...
  fstr.write(textutils.serialiseJSON(fo))
  fstr.close()
...
return ((this.fo.i[fs.combine("", f)].d or this.fo.i[fs.combine("", f)].data ))
end
docker.fs.read = function (this, f,a)
if this.fs.exists(this, f) ~= true then
  return nil
end
...
return string.sub((this.fo.i[fs.combine("", f)].d or this.fo.i[fs.combine("", f)].data ),a or 1)
end
...
this.fo = textutils.unserializeJSON(fstr.readAll())
fstr.close()
local currentInput = stdin
...
env = {
    ...
    ["pocket"]    = pocket,
    ["bit"]       = deepcopy(bit),
    ["bit32"]       = deepcopy(bit32),
    ["help"]      = deepcopy(help),
    --["multishell"] = deepcopy(multishell),
    ["paintutils"] = paintutils,
    ["peripheral"] = deepcopy(peripheral),
    ['redstone'] = deepcopy(redstone),
    ['rs'] = deepcopy(rs),
    ["debug"]     = debug, -- reset it.
    ...
    ["fs"] = {
      open = function(file, mode)
        ...
        local oFile = file
        local oMode = mode
        function fobj.read(i)
          this.fs.read(this,file,i)
        end
        ...
        function fobj.close()
          ...
          fstr.write(textutils.serialiseJSON(this.fo))
          fstr.close()
...
local fileCont = this.fs.readAll(this, this.fo.init)
local entryCode = loadstring(fileCont)

setfenv(entryCode, env)

-- call it.

this:dPrint("init: execute "..tostring(this.fo.init))
local ok,err = pcall(parallel.waitForAny,entryCode,function()
  while pwr do
    coroutine.yield()
  end
end)
if not ok then
  logger.FATA("[0015] "..err)
  logger.fail('[0014] Container Error')
  local f = fs.open(string.format('var/ccdocker/dumps/Launch.LuaDump'),'w')
  f.write(fileCont)
  f.close()
else
  logger.success('[0013] Container Success')
end
end
...
if fs.exists(fs.combine(dir, "rootfs.ccdocker.fs")) then
  logger.NOTI("[0016] removing old fs")
  ...
  function walk(direct, from, to)
    ...
    for k, c in pairs(fs.list(direct)) do
      if fs.isDir(fs.combine(direct, c)) ~= false then
        walk(fs.combine(direct, c), from, fs.combine(to,c))
      else
        local a = string.sub(direct, #dir, #direct)
        local b = string.gsub(a,"/"..from,to)
        local b = fs.combine(to,c)
        local d = string.sub(b,1,#b-#c)
        table.insert(files, {
          odir = direct,
          rdir = d,
...
ffh.write(textutils.serialiseJSON(this.fo))
ffh.close()
